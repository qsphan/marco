...
def main():
    ...
    # introduce a notion of background consrtaints.
    # introduce N(v,w) or ~N(v,w)
    # introduce the set of guard predicates (n*2^m predicates). 
    csolver = SubsetSolver(constraints)
    msolver = MapSolver(constraints)
...
class MapSolver:
   def __init__(self, constraints):
       ...
       # Initialize the state of the solver to know SMT:
       for i in range(len(constraints)):
             self.solver.add(constraints[i] == Bool(str(i)))
